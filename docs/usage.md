...
[settings](settings.md), and [the command-line guide](cli.md).
...
Development uses versioned paths for the terminal forks and Windows privacy
adapter under `vendor/`. Registry installs use their published equivalents.
See [release instructions](release.md) for offline builds and verified archives.
...
[the terminal matrix](terminal-matrix.md).

## Verification and release status

Automated reducer/property, Unicode, configuration, storage, CLI, snapshots and
PTY suites are included. [The coverage ledger](coverage.md) tracks every
specification requirement separately from its evidence. [Measurements](measurements)
