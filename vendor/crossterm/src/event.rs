...
/// A cloneable handle that wakes the sole `poll` owner without reading input.
/// Obtain it on the input thread before entering a blocking poll. Do not mix
/// this polling interface with EventStream. Waking makes `poll` return false
/// when no keyboard event is available; it never inserts a synthetic key.
...
#[cfg(feature = "poll-waker")]
impl PollWaker {
    /// Wake the input owner's current or next poll. Safe to call from another
    /// ordinary thread; not intended for asynchronous signal handlers.
    pub fn wake(&self) -> std::io::Result<()> {
        self.inner.wake()
    }
}

/// Initialize and obtain the input reader's external wake handle. Call once on
/// the same thread that owns all subsequent `poll` and `read` calls.
#[cfg(feature = "poll-waker")]
pub fn poll_waker() -> std::io::Result<PollWaker> {
    lock_internal_event_reader().try_waker().map(|inner| PollWaker { inner })
}
...
bitflags! {
    ...
    pub struct KeyboardEnhancementFlags: u8 {
        ...
        /// Report Unicode text alongside key metadata. Text is delivered once through
        /// [`Event::KeyWithText`] instead of a separate [`Event::Key`].
...
#[cfg_attr(not(any(feature = "bracketed-paste", feature = "associated-text")), derive(Copy))]
...
pub enum Event {
    ...
    FocusLost,
    /// A supported Unix process signal routed through the reader's safe self-pipe.
    /// Applications decide how to restore terminal state and terminate or suspend.
    #[cfg(all(unix, feature = "managed-signals"))]
    Signal(i32),
    /// A single key event with additional pressed modifiers.
    Key(KeyEvent),
    /// Unicode text associated with a key event. Applications must insert `text`
    /// instead of the key code, and must ignore release events for text input.
    /// Parsed text is bounded to 16 KiB; application-specific content validation
    /// still applies. Key metadata retains modifiers and press/repeat/release kind.
    #[cfg(feature = "associated-text")]
    KeyWithText { key: KeyEvent, text: String },
...
impl Event {
    ...
    pub fn is_key_press(&self) -> bool {
        self.as_key_event().map_or(false, |key| key.kind == KeyEventKind::Press)
    }

    /// Returns `true` if the event is a key release event.
    #[inline]
    pub fn is_key_release(&self) -> bool {
        self.as_key_event().map_or(false, |key| key.kind == KeyEventKind::Release)
    }

    /// Returns `true` if the event is a key repeat event.
    #[inline]
    pub fn is_key_repeat(&self) -> bool {
        self.as_key_event().map_or(false, |key| key.kind == KeyEventKind::Repeat)
    }
    ...
    pub fn as_key_event(&self) -> Option<KeyEvent> {
        match self {
            ...
            #[cfg(feature = "associated-text")]
            Event::KeyWithText { key, .. } => Some(*key),
            _ => None,
        }
    }
    ...
    pub fn as_key_press_event(&self) -> Option<KeyEvent> {
        self.as_key_event().filter(|key| key.kind == KeyEventKind::Press)
    }
    ...
    pub fn as_key_release_event(&self) -> Option<KeyEvent> {
        self.as_key_event().filter(|key| key.kind == KeyEventKind::Release)
    }
    ...
    pub fn as_key_repeat_event(&self) -> Option<KeyEvent> {
        self.as_key_event().filter(|key| key.kind == KeyEventKind::Repeat)
    }
...
/// Prepare signal-only delivery before entering raw mode. This does not read or
/// initialize keyboard input; the input owner later adopts the self-pipe.
#[cfg(all(unix, feature = "managed-signals"))]
pub fn prepare_signals() -> std::io::Result<()> { source::unix::prepare_signals() }

/// Discard pending keyboard input and partial parser state at an epoch barrier.
/// Only the thread that owns `poll`/`read` may call this. Signals and geometry
/// notifications remain observable. This does not read terminal responses.
#[cfg(feature = "poll-waker")]
pub fn discard_pending_input() -> std::io::Result<()> {
    lock_internal_event_reader().discard_pending_input()
}

/// A response observed only by an explicitly requested diagnostic probe.
/// Flags are currently active keyboard flags; they are not a list of every
/// enhancement the terminal can support. Ordinary input remains separately typed.
#[cfg(all(unix, feature = "poll-waker"))]
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum KeyboardProbeEvent {
    /// An ordinary input or managed signal event received during the probe.
    Input(Event),
    /// A response to CSI ? u.
    Flags(KeyboardEnhancementFlags),
    /// A response to primary device attributes (CSI c).
    PrimaryDeviceAttributes,
    /// An unrelated terminal response. Its bytes are never exposed as text.
    Other,
}

/// Poll one diagnostic response on the sole input-owner thread. This function
/// sends no query, changes no modes, and does not access stdout. The caller
/// supplies a total deadline and controls the terminal writer/raw-mode guard.
/// Do not use concurrently with `read`, `poll`, or EventStream.
#[cfg(all(unix, feature = "poll-waker"))]
pub fn poll_keyboard_probe(timeout: Duration) -> std::io::Result<Option<KeyboardProbeEvent>> {
    struct EveryEvent;
    impl Filter for EveryEvent {
        fn eval(&self, _: &InternalEvent) -> bool { true }
    }
    let budget = PollTimeout::new(Some(timeout));
    let Some(mut reader) = try_lock_internal_event_reader_for(timeout) else {
        return Ok(None);
    };
    if !reader.poll(budget.leftover(), &EveryEvent)? { return Ok(None); }
    Ok(Some(match reader.read(&EveryEvent)? {
        InternalEvent::Event(event) => KeyboardProbeEvent::Input(event),
        InternalEvent::KeyboardEnhancementFlags(flags) => KeyboardProbeEvent::Flags(flags),
        InternalEvent::PrimaryDeviceAttributes => KeyboardProbeEvent::PrimaryDeviceAttributes,
        InternalEvent::CursorPosition(_, _) => KeyboardProbeEvent::Other,
    }))
}
