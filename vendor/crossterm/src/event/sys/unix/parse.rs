...
fn could_not_parse_event_error() -> io::Error {
    ...
    io::Error::new(kind, "Could not parse an event.")
}
...
fn parse_csi_keyboard_enhancement_flags(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let bits = std::str::from_utf8(digits)
        ...
        .parse::<u8>()
        .map_err(|_| could_not_parse_event_error())?;
    let flags = KeyboardEnhancementFlags::from_bits_retain(bits);
...
pub(crate) fn parse_csi_u_encoded_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    #[cfg(feature = "associated-text")]
    if let Some(codepoints) = split.next() {
        ...
        if !codepoints.is_empty() {
            for codepoint in codepoints.split(':') {
                let scalar = codepoint.parse::<u32>()
                    .ok().and_then(char::from_u32)
                    .ok_or_else(could_not_parse_event_error)?;
                if text.len() + scalar.len_utf8() > 16 * 1024 {
                    return Err(io::Error::new(io::ErrorKind::InvalidData,
                        "associated keyboard text exceeds 16 KiB"));
                }
                text.push(scalar);
            }
        }
        return Ok(Some(InternalEvent::Event(Event::KeyWithText { key, text })));
    }
...
mod tests {
    ...
    #[test]
    fn keyboard_status_reply_uses_complete_decimal_value() {
        for bits in [0_u8, 1, 8, 16, 23, 31, 255] {
            let reply = format!("\x1b[?{}u", bits);
            assert_eq!(parse_csi_keyboard_enhancement_flags(reply.as_bytes()).unwrap(),
                Some(InternalEvent::KeyboardEnhancementFlags(KeyboardEnhancementFlags::from_bits_retain(bits))));
        }
    }

    #[test]
    fn malformed_keyboard_status_reply_is_rejected() {
        for reply in [b"\x1b[?256u".as_slice(), b"\x1b[?1;2u", b"\x1b[?+1u", b"\x1b[?-1u"] {
            assert!(parse_csi_keyboard_enhancement_flags(reply).is_err());
        }
    }
...
mod associated_text_tests {
    use super::*;

    fn text_event(sequence: &[u8]) -> (KeyEvent, String) {
        match parse_csi_u_encoded_key_code(sequence).unwrap().unwrap() {
            InternalEvent::Event(Event::KeyWithText { key, text }) => (key, text),
            other => panic!("expected one associated-text event, got {other:?}"),
        }
    }

    #[test]
    fn associated_text_keeps_physical_key_separate_from_inserted_text() {
        let (key, text) = text_event(b"\x1b[97;1;98u");
    ...
    #[test]
    fn associated_text_preserves_press_repeat_release_and_modifiers() {
        for (code, expected) in [(1, KeyEventKind::Press), (2, KeyEventKind::Repeat), (3, KeyEventKind::Release)] {
            let sequence = format!("\x1b[97;5:{code};98u");
            let (key, text) = text_event(sequence.as_bytes());
    ...
    #[test]
    fn associated_text_preserves_combining_and_multi_grapheme_text() {
        let (_, text) = text_event(b"\x1b[101;1;101:769:32:30028u");
        assert_eq!(text, "e\u{301} 界");
    }

    #[test]
    fn associated_text_empty_payload_does_not_fall_back_to_key_code() {
        let (key, text) = text_event(b"\x1b[97;1;u");
        assert_eq!(key.code, KeyCode::Char('a'));
        assert!(text.is_empty());
    }

    #[test]
    fn associated_text_invalid_scalars_and_fields_are_rejected() {
        for sequence in [
            b"\x1b[97;1;55296u".as_slice(),
            b"\x1b[97;1;1114112u".as_slice(),
            b"\x1b[97;1;xxu".as_slice(),
            b"\x1b[97;1;97::98u".as_slice(),
            b"\x1b[97;1;98;99u".as_slice(),
        ] {
            assert!(parse_csi_u_encoded_key_code(sequence).is_err(), "{sequence:?}");
        }
    }

    #[test]
    fn associated_text_payload_has_a_hard_byte_bound() {
        let points = vec!["128512"; 4097].join(":");
        let sequence = format!("\x1b[97;1;{points}u");
        assert!(parse_csi_u_encoded_key_code(sequence.as_bytes()).is_err());
    }

    #[test]
    fn associated_text_does_not_change_baseline_key_delivery() {
        assert_eq!(parse_csi_u_encoded_key_code(b"\x1b[97u").unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(KeyCode::Char('a'), KeyModifiers::NONE)))));
    }
}
