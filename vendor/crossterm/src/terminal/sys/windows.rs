...
// Preserve both console modes exactly, including user-selected input flags
// and VT processing that command dispatch may enable during the session.
static MODES_PRIOR_RAW: parking_lot::Mutex<Option<(DWORD, DWORD)>> = parking_lot::const_mutex(None);

pub(crate) fn enable_raw_mode() -> std::io::Result<()> {
    let mut prior = MODES_PRIOR_RAW.lock();
    if prior.is_some() { return Ok(()); }
    ...
    input.set_mode(input_mode & !NOT_RAW_MODE_MASK)?;
    *prior = Some((input_mode, output_mode));
    // Command support is cached, so a second session must reenable processing
    // after an earlier session restored the original output flags.
    let _ = crate::ansi_support::enable_vt_processing();
    Ok(())
}

pub(crate) fn disable_raw_mode() -> std::io::Result<()> {
    let mut prior = MODES_PRIOR_RAW.lock();
    let Some((input_mode, output_mode)) = *prior else { return Ok(()); };
    let input = ConsoleMode::from(Handle::current_in_handle()?);
    let input_result = input.set_mode(input_mode);
    let output_result = Handle::current_out_handle()
        .and_then(|handle| ConsoleMode::from(handle).set_mode(output_mode));
    input_result?;
    output_result?;
    *prior = None;
    Ok(())
